...
| 知识索引 | `build-knowledge-index` | 渲染 → 主 |
...
└──────────────┘    └──────┬───────┘    └──────────────┘
                    ...
                    └──────┬───────┘
                           ...
                           ▼
┌──────────────┐    ┌──────────────┐    ┌──────────────┐
│KnowledgeIndex│───▶│KnowledgeRetr │◀───│  ToolRouter  │
│ (离线建索引) │    │ (BM25+向量)  │    │ (工具检索)   │
└──────────────┘    └──────┬───────┘    └──────────────┘
                           │ 检索结果
                           ▼
...
| **chatStore** | `chatStore.ts` | Zustand 状态管理：消息列表、对话发送（含本地边界守卫 + 工具调用循环 + 记忆/画像自动更新 + 技能建议提取 + 答案自动沉淀） |
...
[渲染进程] chatStore.sendMessage()
  │
  ├─ 1. 本地边界守卫 / cache / 附件块处理
  │
  ├─ 2. 构建 API 消息列表
  │     [system: systemPrompt, ...history, user: contentWithAttachments]
  │
  ├─ 3. 调用 LLM（流式 SSE）
  │     │
  │     └─ 如需知识，模型自主调用 search_knowledge / knowledge_grep / query_excel
...
| **OCR 模型** | 文字识别 / 文档导入辅助 | 通义千问 qwen-vl-ocr |
...
| `createLLMFn()` | 主进程 | 索引构建（上下文摘要）等后台任务 |
...
```
  ...
  knowledge/*.md   ────▶ │  切块 → 上下文摘要(LLM) → 向量嵌入      │ ──▶ _index/
                         ...
                         │       阶段三：在线问答（工具检索）        │
                         │                                         │
  用户提问         ────▶ │  LLM 按需调用 search_knowledge /         │ ──▶ 工具结果
                         │  knowledge_grep / query_excel            │     → LLM 回答
                         │  ToolRouter.execute()                    │
                         └─────────────────────────────────────────┘
                                          │
                                          ▼
...
### 8.3 阶段三：在线问答（用户提问 → 工具检索 → 回答）

用户发送消息后，系统不再执行 pre-message 知识注入；渲染层只拼接必要的附件元信息和动态系统段，然后进入 LLM 工具循环。涉及知识库事实、参数、来源或表格数据时，由模型按需调用本地工具获取证据。

#### 8.3.1 两类知识来源

| 通道 | 注入位置 | 机制 | 特点 |
|------|---------|------|------|
| **A: 稳定上下文** | System Prompt | `SoulLoader.loadAvatar()` 注入分身定义、知识索引摘要、技能索引和硬规则 | 稳定、可缓存，不包含每轮临时检索结果 |
| **B: 按需工具检索** | Tool Results | LLM 自主调用 `search_knowledge` / `knowledge_grep` / `query_excel` / `read_knowledge_file` | 只在需要时取证据，结果进入工具消息并可溯源 |
...
用户问题: "ENS-L262 的内部尺寸是多少？能放多少个电芯模组？"
  │
  ▼
Step 1: chatStore.sendMessage()
   ├─ 本地边界守卫优先返回固定口径
   ├─ 命中答案/图表缓存则直接返回
   └─ 构造 systemBlocks + history + user message
  │
  ▼
Step 2: LLM 判断是否需要知识
   ├─ 普通寒暄/承接：不调用知识工具
  ...
  │
  ▼
Step 3: ToolRouter 执行本地检索
   ├─ search_knowledge：按 query 切块检索，返回 coverage 与来源片段
   ├─ knowledge_grep：按精确词/型号/参数名扫描知识库
   ├─ query_excel：读取结构化 Excel/CSV 行级数据
   └─ read_knowledge_file：按文件路径读取指定知识文件
  │
  ▼
Step 4: LLM 基于工具结果收敛回答
   ├─ 关键数据标注来源
   ├─ 检索未命中时只说明本轮未命中，不证明知识库不存在
   └─ 工具达到上限后进入收敛模式，基于已有结果回答
```
...
工具调用是当前知识问答的主路径：LLM 在回答中发现需要更多信息时自主发起，本地 ToolRouter 返回可溯源结果。
...
## 附录：Soul 工具化知识检索与 Karpathy LLM Wiki 方法对比

> Karpathy 于 2026 年 4 月发布了 [LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) 方法论，主张用 LLM 将原始文档"编译"成结构化 Wiki，替代传统检索增强。本节将 Soul 当前的工具化知识检索与该方法做架构级对比。
...
Soul 工具化知识检索:
  原始文档 ──LLM 格式化──▶ 知识文件（保留全文，仅排版）
                          └─ 切块 + BM25/向量索引 ──▶ 工具按需返回来源片段
```

| 维度 | Karpathy LLM Wiki | Soul 工具化知识检索 |
|------|-------------------|-------------------|
...
| **查询机制** | LLM 读 index.md → 找到相关页面 → 直接读取 Wiki 全文 | System Prompt 稳定上下文 + LLM 工具按需检索 |
...
| 对比项 | Karpathy LLM Wiki | Soul 工具化知识检索 |
|--------|-------------------|-------------------|
...
| 对比项 | Karpathy LLM Wiki | Soul 工具化知识检索 |
|--------|-------------------|-------------------|
| 查询流程 | LLM 读 index.md → 选择相关页面 → 读取 → 回答 | LLM 判断需要知识 → 调用 `search_knowledge` / `knowledge_grep` / `query_excel` → 基于工具结果回答 |
| 上下文占用 | 整个 Wiki 内容需放入上下文窗口 | 仅工具命中的片段进入消息历史，节省上下文空间 |
| 跨文档综合 | 天然支持：Wiki 编译时已完成跨文档综合 | 通过多次工具检索与模型综合实现跨文档关联 |
...
7. **按需取证**：专业事实、参数和表格数据通过本地工具返回，减少无关上下文污染
...
| 工程技术文档问答 | **Soul 工具化知识检索** | 需要精确数值、来源可溯、零信息丢失 |
| 产品手册/用户手册 | **Soul 工具化知识检索** | 包含大量图表、参数表、接线图，需 Vision OCR + 数值校验 |
| AI 领域研究综述 | **Karpathy Wiki** | 需要综合 50+ 论文的趋势、对比、演进脉络 |
| 企业知识库（多人协作） | **两者都不够** | Wiki 缺少并发控制，工具化检索缺少知识演化 |
| 政策法规查询 | **Soul 工具化知识检索** | 需要精确引用原文条款，不容忍任何改写 |
...
| 工具化知识检索 | 不受影响 | 依赖 KnowledgeRetriever，不接触 `wiki/` |
...
百科功能通过手动触发或自动沉淀启用，不再提供“回答时自动注入百科”的 pre-message 链路。当前保留的是概念页编译、答案沉淀、演化检测和 lint；回答阶段如需知识，仍由工具检索路径按需读取知识库。

##### 已移除：回答前自动注入百科

旧版本曾在回答前把 `wiki/concepts/` 的概念页作为补充参考拼入用户消息。该路径已随旧程序化检索链路下线，设置页中的对应开关也已移除。
...
导入文档 → ┬→ 原始文件 → knowledge/_raw/xxx.pdf       ← 可追溯
  ...
  回答阶段:
    用户问题 → LLM 按需调用知识工具 → ToolRouter 返回来源片段/表格数据
            → LLM 基于工具结果回答

  答案沉淀（功能 2）:
    手动 SAVE / 自动判断（wiki_auto_sediment=true）→ wiki/qa/*.md

              ↑ 百科编译/沉淀均不改变 knowledge/ 中的任何 .md 文件
              ↑ 自动沉淀需要设置开关开启才生效
```
