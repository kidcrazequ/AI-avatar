...
Codex 会自动加载该分身的 AGENTS.md → soul.md → knowledge/ → memory/，以该分身的身份与你交互。
...
AGENTS.md ←── templates/agent-template.md   （操作规则：知识库约束、工作流程、第一性原理）
...
3. **读取** `templates/agent-template.md` 创建 `AGENTS.md`（包含知识库约束、第一性原理、工作流程）
...
- 改动仅涉及非代码文件（`.md` / `.json` / 配置文件 / AGENTS.md / 知识文件等）
- 任何不涉及代码修改的对话
...
| "为什么我的 AI 分身不像 Codex" | ⛔ 不触发 | 调研/解释类问题，直接回答 |
