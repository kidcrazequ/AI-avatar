...
- **自包含 ZIP 分身包** — 导入 / 导出面板支持 `.soulpack.zip`，将人设、技能、知识、可选记忆与 Excel / PDF / 图片等二进制附件一并无损分发；原 `.soul.json` 文本包继续兼容。
- **普通分身目录 ZIP 兼容导入** — 可直接导入「完整分身目录压缩包」，自动识别单顶层分身、转换为标准 Soul Pack，并过滤 `__MACOSX`、`workspaces/`、`_index/` 和 `knowledge.backup-*` 等杂项。

### 安全加固

- **ZIP 导入完整性与资源防护** — 二进制附件逐文件校验 SHA256，预览与导入使用 manifest 指纹防 TOCTOU，并增加压缩 / 解压体积、文件数、路径穿越与多顶层目录校验。

## v0.19.0 (2026-07-08)

### 新增

- **职场环境记忆（Palace）** — 新增分身级 `palace/` 文件协议，把“任务前该想起什么”做成可路由的职业处境层：路线卡、任务前上下文包、承诺台账、任务后 inbox 沉淀、ToolRouter 工具入口与桌面端“职场”面板；配套新增资料 tab、档案/承诺写入 IPC、存量分身回填脚本与 `docs/palace-guide.md` 协议指南。
- **工作流技能沉淀闭环** — 新增对话蒸馏工作流技能模块、skill-index 收录 API、草稿晋升/删除/列举 IPC，以及桌面端“对话沉淀 / 草稿晋升 / 一键运行”三入口。
- **三级技能体系与社区技能兼容** — `soul-sync` 支持目录型 SKILL.md 技能零转换安装，新增技能体系只读校验器、社区技能目录安装修复、存量 description 触发条件优先重写，以及无技能/有技能对照评测 `skill-ab`。
- **Agent runtime 与运行时观测** — 接入 agent-runtime 到实时对话，补 run-trace / verifier / workspace 观测层；新增工具调用安全的主动上下文压缩、单轮成本预算上限、exec_shell 放行常用 agent 工具子命令，并支持“本次会话始终允许”的灰名单确认。
- **分身办公室与设计资产** — 新增实验性“分身办公室”可视化页、等距 3D 办公室原型与品牌吉祥物资产；补行业设计决策表与 design-master 查表技能。

### 修复

- **聊天流式与状态一致性** — thinking 与正文改为全轮次实时流式；缺 apiKey 早退前从设置重读自愈；发送链路新增四级观测埋点和渲染层异常兜底；chatStore HMR 改整页刷新，避免 store 裂脑。
- **构建与脚本稳健性** — Vite 改为直读 `@soul/core/browser` TS 源码，修复链接包缓存脱节；`parse_sources` 改为零依赖受限 YAML 子集解析；上下文压缩在负收益膨胀时整体回退。

### 工程 / 文档

- **技能与模板治理** — 模板对齐 agentskills.io 目录型技能格式，补判断力规则化说明；记录社区热门 AI 项目深读、SDK 借鉴 backlog、BR-6 不做决策与技能索引/记忆四分边界。
- **长尾资料转换** — 补充 markitdown 长尾转换脚本，覆盖 EPUB / docx / pptx 缺口。
...
- **企业微信聊天截图导入** — 知识库面板支持导入企业微信聊天截图，转成可检索知识内容。
- **来源引用面板内查看** — Markdown 来源 chip 改为在应用内知识库面板打开，不再跳出当前阅读上下文。
...
- **小凯知识摄取溯源路径** — 摄取脚本生成相对原始目录路径，避免分身包分发后暴露或依赖本机绝对路径。
