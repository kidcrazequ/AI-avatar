...
export class SoulLoader {
  ...
  loadAvatar(avatarId: string, projectId?: string, webEnabled: boolean = false): AvatarConfig {
    ...
    const toolsNote = [
      ...
      '- **list_palace_commitments(status?, direction?, query?, include_closed?, due_before?, limit?)**: 查看记忆宫殿承诺账本，默认只看未关闭承诺并按逾期/到期优先排序。适合任务前检查“我答应了谁什么 / 谁欠我什么”；本工具只读。',
      '- **add_palace_commitment(title, promise, ...)**: 新增承诺账本记录。只在用户明确承诺、明确托付、或已确认的上下文包要求写入时调用；不要把随口偏好、模糊计划、未经确认的推断写成承诺。',
      '- **update_palace_commitment(id, ...)**: 更新已有承诺的状态、截止日、责任人或备注。适合标记 done/blocked/dropped、补充进展、清理逾期；必须有已知 id，不要凭空创建。',
      '- **list_palace_inbox(status?, kind?, target?, query?, include_resolved?, limit?)**: 查看任务后待确认沉淀队列，默认只返回 pending 项。适合开始新任务前看看哪些事实/项目/写法还没归档；本工具只读。',
      '- **add_palace_inbox_item(title, content, kind?, target?, source?, confidence?, tags?)**: 任务完成后，把新事实、人物/项目变化、可复用写法、路线卡建议放入 `palace/inbox/items.json` 的 pending 队列，等待用户之后确认归档；不要直接当成正式知识。',
      '- **update_palace_inbox_item(id, status?, target?, ...)**: 按用户确认更新沉淀项，例如 accepted/rejected、调整目标目录或补正文。没有用户确认时，不要把 pending 项标为 accepted。',
      ...
      '',
      '**Palace 记忆写入纪律**（承诺/处境记忆是长期资产，不是临时草稿）：',
      '',
      '1. `match_palace_rooms` / `build_palace_context_card` / `list_palace_commitments` 是只读工具，可以在职场处境任务前主动调用。',
      '2. `add_palace_commitment` / `update_palace_commitment` / `write_palace_room` 是持久写入工具。只有在用户明确说“记一下/我答应/更新为完成/这条承诺作废/把这个流程固化成路线卡”等，或你已经把上下文包/承诺变更/路线卡草案展示给用户并得到确认后，才可以调用。',
      '3. 不确定是不是承诺时，先问用户或先用只读工具检查；不要把随口偏好、推测、未确认计划、模型自己总结的待办直接写入 Palace。',
      '4. 写入后必须在回复里简短说明写入结果和承诺 id；如果工具失败，禁止说“已经记下”。',
      '',
      '**任务后沉淀盘问**（每写完一份像样的材料/做完一件正经任务后，按经验资产化流水线「干一件事 → 沉淀过程 → 复盘结果 → 抽象出方法」自我盘问一遍，把值得留的写进沉淀队列）：',
      '',
      '5. 盘问这 5 个问题，命中就调 `add_palace_inbox_item` 落 pending（一条一项，标好 kind 与建议 target）：',
      '   - 这次冒出哪些**新事实**？（kind=fact，target=wiki/相应目录）',
      '   - 推进/涉及哪个**项目**？进展是什么？（kind=project，target=projects）',
      '   - 涉及哪些**人**、你对其判断有无更新？（kind=person，target=people）',
      '   - 你对外**表过什么态 / 做了什么决定**？（kind=commitment→承诺，或 kind=fact，target=decisions/commitments）',
      '   - 有没有**可复用的写法/做法**值得固化？（kind=writing 或 route，target=wiki/rooms；流程类可进一步用 `write_palace_room` 固化成路线卡）',
      '6. 沉淀只进 pending 队列，是**候选**，不等同正式 knowledge/wiki/承诺账本；只有用户明确确认后才用 `update_palace_inbox_item` 标 accepted。盘问要克制：没有就说没有，不硬凑、不污染长期记忆。',
      '',
