...
function initManagers() {
  ...
  // F2 实时接线（借鉴 Pi）：SOUL_USE_NEW_RUNTIME 开启时给 toolRouter 挂**只软警告**的来源锚点
  // hook（不挂 deny 类，避免误拦正常工具）。命中即记日志，提示取数结果缺 [来源: ...] 锚点。
  // 默认 flag 关 → 不构造 registry、execute 内 this.hooks 为空 → 行为与此前完全一致。
  if (AgentRuntime.isNewRuntimeEnabled()) {
    const hookRegistry = new AgentRuntime.HookRegistry()
    // hook 内部异常默认被 registry 静默吞掉（"不影响主流程"）；这里接出来记日志，便于排查坏 hook。
    hookRegistry.setOnHandlerError((err, ctx) => {
      const msg = err instanceof Error ? err.message : String(err)
      if (logger) logger.activity('hook-error', `${ctx.hookId}@${ctx.point}: ${msg}`)
      else console.warn(`[hook-error] ${ctx.hookId}@${ctx.point}: ${msg}`)
    })
    const anchorHook = AgentRuntime.makeSourceAnchorEnforcementHook({
      onWarning: (w) => {
        if (logger) logger.activity('source-anchor-warning', `${w.toolName}: ${w.reason}`)
        else console.warn(`[source-anchor] ${w.toolName}: ${w.reason}`)
      },
    })
    hookRegistry.register(AgentRuntime.HookPoint.POST_TOOL_USE, anchorHook.handler, 'source-anchor-enforcement')
    toolRouter.setHooks(hookRegistry)
    if (logger) logger.activity('agent-runtime', 'source-anchor 软警告 hook 已挂载到实时工具路径（SOUL_USE_NEW_RUNTIME）')
  }
...
// messageId 为空字符串 = 分叉到根（首轮换个思路），故仅在非空时做路径段校验。
wrapHandler('fork-conversation', (_, conversationId: string, messageId: string) => {
  assertSafeSegment(conversationId, '会话 ID')
  if (messageId) assertSafeSegment(messageId, 'messageId')
  return getDb().forkConversationFromMessage(conversationId, messageId)
})

// 会话树轻量结构（v21·phase2）：供渲染端算分叉点/版本数/分支切换目标。
wrapHandler('get-conversation-tree', (_, conversationId: string) => {
  assertSafeSegment(conversationId, '会话 ID')
  return getDb().getConversationTree(conversationId)
})
...
// renderer 可以传任意绝对路径让主进程 writeFileSync——一旦渲染层被污染，
// 这就是任意文件写。现在改成主进程主导对话框：
//   - export：主进程 showSaveDialog，用户挑路径，主进程才落盘
//   - preview：主进程 showOpenDialog，读 + parse + 发放一次性 path token
//   - import：renderer 拿 token 调，主进程根据 token 反查路径再读
//
// path token 5 分钟过期 + 用一次销毁，避免 renderer 缓存 token 重复操作；
// 同时 preview 必须先跑，import 才能拿到对应 token——杜绝直接传任意路径的入口。
...
/**
 * preview：主进程开 showOpenDialog 让用户挑一个 .soul.json，
 * 读 + parse 校验通过后发放一次性 path token——后续 import 必须用此 token。
 * 渲染层永远拿不到、也不需要绝对路径。
 */
wrapHandler('soul-pack:preview', async () => {
  const win = mainWindow ?? BrowserWindow.getFocusedWindow()
  if (!win) {
    return { ok: false as const, canceled: true, error: '无活动窗口' }
  }
...
wrapHandler('palace:add-commitment', async (
  _,
  avatarId: string,
  input: { title: string; promise: string; counterparty?: string; direction?: string; dueAt?: string },
) => {
  assertSafeSegment(avatarId, '分身ID')
  if (!input || typeof input.title !== 'string' || typeof input.promise !== 'string') {
    throw new Error('palace:add-commitment 需要 title 和 promise')
  }
  return addPalaceCommitment(avatarsPath, avatarId, input as never)
})
...
// 「资料」面板只管这 7 个分类文档目录；rooms/inbox/profile/company 有各自入口，不走这里。
const PALACE_DOC_DIRS = new Set(['people', 'projects', 'meetings', 'reports', 'decisions', 'achievements', 'wiki'])
function assertPalaceDocDir(dir: string): asserts dir is 'people' | 'projects' | 'meetings' | 'reports' | 'decisions' | 'achievements' | 'wiki' {
  if (!PALACE_DOC_DIRS.has(dir)) throw new Error(`非法资料目录: ${dir}`)
}

wrapHandler('palace:list-dir-files', async (_, avatarId: string, dir: string) => {
  assertSafeSegment(avatarId, '分身ID')
  assertPalaceDocDir(dir)
  return listPalaceDirectoryFiles(avatarsPath, avatarId, dir)
})

wrapHandler('palace:read-dir-file', async (_, avatarId: string, dir: string, name: string) => {
  assertSafeSegment(avatarId, '分身ID')
  assertPalaceDocDir(dir)
  return readPalaceDirectoryFile(avatarsPath, avatarId, dir, name)
})

wrapHandler('palace:write-dir-file', async (_, avatarId: string, dir: string, name: string, content: string) => {
  assertSafeSegment(avatarId, '分身ID')
  assertPalaceDocDir(dir)
  if (typeof content !== 'string') throw new Error('content 必须是字符串')
  await writePalaceDirectoryFile(avatarsPath, avatarId, dir, name, content)
})

wrapHandler('palace:delete-dir-file', async (_, avatarId: string, dir: string, name: string) => {
  assertSafeSegment(avatarId, '分身ID')
  assertPalaceDocDir(dir)
  await deletePalaceDirectoryFile(avatarsPath, avatarId, dir, name)
})
...
// 借鉴 Pi 的可脚本化 seam：把内置 stdio MCP server 的接线做成"一键配置"，让外部工具/
// 定时任务能 query 某分身知识（list_avatars / search_chunks 等只读工具已就绪）。
// 完整 ask_avatar（headless 跑 ToolRouter+LLM）较重，暂不在此实现——只读检索已覆盖外部查询。
wrapHandler('mcp:generate-settings-snippet', () => {
  // dev：core dist 在 <soul>/packages/core/dist（与 resolveExpertPacksPath 同款 __dirname 推导）；
  // 打包后 core 通常已并入主包、独立 bin 可能不随包分发，故标 binResolved 供 UI 决定是否提示。
  const devBin = path.join(__dirname, '../../packages/core/dist/mcp-server-bin.js')
  const binResolved = fs.existsSync(devBin)
  const binPath = binResolved ? devBin : '<soul>/packages/core/dist/mcp-server-bin.js'
  const snippet = buildMcpServerSettingsSnippet({ binPath, avatarsPath })
  return { ...snippet, binResolved }
})
...
wrapHandler('mcp:upsert-server', async (_event, input: McpServerInput) => {
  ...
  // 0. 写 DB 前完整校验配置（transport/command/url）：避免 IPC 绕过 UI 传入坏配置
  //    （如 stdio 缺 command）被持久化后，下次启动构造 transport 时抛错污染连接路径。
  const validationError = validateMcpServerConfig({
    name: input.name,
