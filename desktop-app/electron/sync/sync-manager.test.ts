...
test('sync-manager: applyExtractedSnapshot DB 替换后某步失败 → DB 回滚到恢复前', { skip: skipReason ?? false }, async () => {
  ...
  try {
    ...
    // 回滚后 DB 旧库暂存不应残留
    assert.ok(
      !fs.readdirSync(h.workDir).some((n) => n.startsWith('xiaodu.db.restore-bak.')),
      '回滚后 xiaodu.db.restore-bak.* 不应残留',
    )
  } finally {
    h.cleanup()
  }
...
test('sync-manager: applyExtractedSnapshot 全链路成功 → 不遗留任何 .restore-bak（DB + 目录）', { skip: skipReason ?? false }, async () => {
  resetFakeWebDav()
  const h = makeManager()
  try {
    // 预置旧 DB；本地 avatars/a.txt 由 makeManager 预置
    const targetDb = path.join(h.workDir, 'xiaodu.db')
    fs.writeFileSync(targetDb, Buffer.from('OLDDB'))
    ...
    fs.mkdirSync(path.join(snapshotRoot, 'avatars'), { recursive: true })
    fs.writeFileSync(path.join(snapshotRoot, 'avatars', 'remote.txt'), Buffer.from('REMOTE'))
    fs.mkdirSync(path.join(snapshotRoot, 'shared'), { recursive: true })
    fs.writeFileSync(path.join(snapshotRoot, 'shared', 's.txt'), Buffer.from('S'))
    fs.mkdirSync(path.join(snapshotRoot, 'conversations'), { recursive: true })
    fs.writeFileSync(path.join(snapshotRoot, 'conversations', 'c.jsonl'), Buffer.from('{}'))
    ...
    await apply(extractedDir)

    // DB 已替换为远端
    assert.equal(fs.readFileSync(targetDb, 'utf-8'), 'NEWDB', '成功后 DB 应为远端快照')
    // 成功后不得遗留任何 .restore-bak（DB 暂存 + 各目录暂存）
    const leftovers = fs
      .readdirSync(h.workDir)
      .filter((n) => n.includes('.restore-bak.'))
    assert.deepEqual(leftovers, [], `成功路径不应遗留暂存：${leftovers.join(', ')}`)
  } finally {
    h.cleanup()
  }
})
...
test('sync-manager: applyExtractedSnapshot avatars 已替换后 conversations 失败 → avatars + DB 一并回滚', { skip: skipReason ?? false }, async () => {
  ...
  try {
    ...
    // stageDir 检测到「目标存在但非目录」会直接抛错 —— 此时 avatars 已被替换成远端内容。
    fs.writeFileSync(path.join(h.workDir, 'conversations'), Buffer.from('blocker'))
    ...
    // 关键断言 4：作为 blocker 的 conversations 文件未被误删（stageDir 非目录时只抛不删）
    assert.equal(
      fs.readFileSync(path.join(h.workDir, 'conversations'), 'utf-8'),
      'blocker',
      'stageDir 检测到目标非目录应直接抛错，不得删除该原文件',
    )
  } finally {
    h.cleanup()
  }
...
// restore 成功状态丢失修复（#16）：restoreFrom 成功后 DB 已被换成远端快照，旧 db 句柄
// 写不进新库，成功状态落到 restore-pending.json sidecar，由下次启动补记进新库。
// 本用例验证补记把状态写进「当前（新）库」、用新库自增 id（而非旧库的 historyId）、并消费 sidecar。
test('sync-manager: consumePendingRestoreState 把 restore-pending sidecar 补记进新库并消费', { skip: skipReason ?? false }, async () => {
  if (!DatabaseCtor || !SyncManagerMod || !SyncHistoryStoreMod) throw new Error('依赖未加载（应已 skip）')
  const db = new DatabaseCtor(':memory:')
  setupSchema(db)
  const history = new SyncHistoryStoreMod.SyncHistoryStore(db)
  const userDataPath = makeTempDir('restore-backfill')
  const warns: string[] = []
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        durationMs: 1234,
        remoteFilename: 'soul-backup-20260530.zip',
        fileCount: 42,
        totalBytes: 99,
      }),
      'utf-8',
    ...
    // 1. last_sync_* 落到当前库，key 与 persistLastSync 对齐；error 清空
    ...
    assert.equal(getSetting('webdav_last_sync_at'), '1700000000000')
    assert.equal(getSetting('webdav_last_sync_status'), 'success')
    assert.equal(getSetting('webdav_last_sync_direction'), 'restore')
    assert.equal(getSetting('webdav_last_sync_error'), undefined)

    // 2. 新库里有且仅有一条 restore success 历史，字段来自 sidecar，用新库自增 id
    const rows = history.list({ direction: 'restore', status: 'success' })
    assert.equal(rows.length, 1)
    assert.equal(rows[0].file_count, 42)
    assert.equal(rows[0].total_bytes, 99)
    assert.equal(rows[0].duration_ms, 1234)
    assert.equal(rows[0].remote_filename, 'soul-backup-20260530.zip')
    assert.equal(rows[0].created_at, 1_700_000_000_000)

    // 3. sidecar 已消费，warn 无
    assert.ok(!fs.existsSync(sidecar), 'sidecar 必须在补记后删除')
    assert.equal(warns.length, 0)

    // 4. 幂等：无 sidecar 再调一次不新增历史
    SyncManagerMod.SyncManager.consumePendingRestoreState({ db, syncHistoryStore: history, userDataPath, logger })
    assert.equal(history.list({ direction: 'restore', status: 'success' }).length, 1)
  } finally {
    db.close()
...
// sidecar 字段非法（finishedAtMs 非有限数）：不得污染新库的状态，但仍要消费 sidecar 并 warn，
// 否则坏文件会每次启动反复触发解析失败。
test('sync-manager: consumePendingRestoreState 非法 sidecar 不写库、删除 sidecar、warn', { skip: skipReason ?? false }, async () => {
  if (!DatabaseCtor || !SyncManagerMod || !SyncHistoryStoreMod) throw new Error('依赖未加载（应已 skip）')
  const db = new DatabaseCtor(':memory:')
  setupSchema(db)
  const history = new SyncHistoryStoreMod.SyncHistoryStore(db)
  const userDataPath = makeTempDir('restore-backfill-bad')
  const warns: string[] = []
  const logger = { info: () => undefined, warn: (m: string) => warns.push(m) }
  try {
    const sidecar = path.join(userDataPath, 'restore-pending.json')
    fs.writeFileSync(sidecar, JSON.stringify({ status: 'success', finishedAtMs: 'not-a-number' }), 'utf-8')

    SyncManagerMod.SyncManager.consumePendingRestoreState({ db, syncHistoryStore: history, userDataPath, logger })

    assert.equal(history.count({ direction: 'restore' }), 0, '非法 sidecar 不应插入历史')
    assert.equal(
      db.prepare('SELECT value FROM settings WHERE key = ?').get('webdav_last_sync_status'),
      undefined,
      '非法 sidecar 不应写 last_sync_*',
    )
    assert.ok(!fs.existsSync(sidecar), '非法 sidecar 也必须被消费删除')
