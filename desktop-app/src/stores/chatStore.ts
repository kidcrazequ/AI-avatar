...
export const useChatStore = create<ChatStore>((set, get) => ({
  ...
  resetTransientState: () => {
    ...
    set({ isLoading: false, toolCallStatus: '', skillProposals: [], tasks: [] })
  },
  ...
  sendMessage: async (content: string, conversationId: string, avatarId: string, images?: string[], visionModel?: ModelConfig) => {
    ...
    const userMessage: ChatMessage = { id: nextMessageId(), role: 'user', content: taggedContent }
    set({ messages: [...messages, userMessage] })
    try {
      await window.electronAPI.saveMessage(conversationId, 'user', taggedContent, undefined, images)
    } catch (error) {
...
4) 使用已有数据直接收敛输出，不要重复调用同参数工具。`
    ...
    const apiMessages: LLMMessage[] = [
      { role: 'system', content: effectiveSystemPrompt + snipNoticeBlock },
