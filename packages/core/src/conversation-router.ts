 ...
 *       1) 模型选择（图片必须走 vision）
 *       2) 极简输入跳过工具检索（"好的"/"嗯" 之类的纯确认句，避免空调用浪费 token）
...
/**
 * 上下文策略——只剩两个值。
 *   - 'skip-tools' : router 已确定无需工具检索（图片 / 纯确认 / 空输入）
 *   - 'auto'   : 默认；让 LLM 通过工具调用自主决定是否检索
 */
export type ContextStrategy = 'skip-tools' | 'auto'
 ...
 * 命中后跳过工具检索，避免空调用浪费 token。这是少数 router 能判断对的低风险场景。
 */
...
export function routeConversation<TModel>(options: RouteConversationOptions<TModel>): RoutingDecision<TModel> {
  ...
  // 护栏 2：极短输入或纯确认句，跳过工具检索（拿来检索也没意义）
  if (trimmed.length === 0 || trimmed.length < minQueryLength || ACK_REGEX.test(trimmed)) {
    return {
      model: chatModel,
      modelKind: 'chat',
      contextStrategy: 'skip-tools',
