...
test('⑨ getConversationTree 轻量树 + root-fork（空 messageId 清空 leaf）', { skip: skipReason ?? undefined }, () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'soul-db-tree-tree-'))
  const db = new DatabaseManagerCtor!(path.join(dir, 't.db'))
  const conv = db.createConversation('会话', 'avatar-x')
  const a = db.saveMessage(conv, 'user', '一')
  const b = db.saveMessage(conv, 'assistant', '二')
  const tree = db.getConversationTree(conv)
  assert.equal(tree.length, 2)
  assert.equal(tree.find((n) => n.id === b)!.parentId, a, '树带 parentId')
  assert.ok(typeof tree[0].createdAt === 'number' && tree[0].role === 'user')
  // root-fork：空 messageId → leaf 清空（首轮"换个思路"用）
  assert.equal(db.forkConversationFromMessage(conv, ''), true)
  assert.equal(db.getConversation(conv)!.leaf_message_id, null, 'root-fork 应清空 leaf')
  db.close()
})
