/**
 * 知识检索策略决策（借鉴 Pi Coding Agent 的渐进式披露）。
 *
 * 现状（重要）：chat 路径已于 2026-05-13 改为 agentic-only —— pre-message BM25 注入
 * 早已删除，知识只通过 search_knowledge 工具按需进入。因此 F7 借鉴点的"删除 pre-message
 * RAG"半边已落地，无需重做。
 *
 ...
 * 敏感的 Phase-2，须 eval 守门，由 SOUL_USE_PROGRESSIVE_DISCLOSURE_RAG 控制（默认关）。
 *
 * @author zhi.qu
 * @date 2026-06-01
 */
...
export interface DecideKnowledgeStrategyOptions {
  ...
  /** 渐进式披露 RAG 是否启用（来自 isProgressiveDisclosureRagEnabled），缺省 false。 */
  enabled?: boolean
}
