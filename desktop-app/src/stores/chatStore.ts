...
// HARD_RULES / DELIBERATION_GUIDE / buildStableSystemText 已抽到 ./stable-system-prompt
// （prompt-cache 稳定前缀的唯一正本，配 stable-system-prompt.test.ts 回归保护）
...
// BR-2: 主动上下文压缩的保留参数（仅当 compaction_trigger_input_tokens>0 时启用）
const COMPACTION_RETAIN_TAIL = 8 // 保留最近 ≥8 条（实际 snap 到 user 边界，通常更多）
const COMPACTION_MIN_MIDDLE = 6 // 中段至少 6 条才值得摘要，避免小规模无谓压缩
...
[系统提示] 用户在消息中使用了 @web，表示希望你本轮主动联网：请优先调用 web_search（必要时配合 web_fetch）获取实时信息后再作答，并在回答中标注来源链接。@web 只是触发联网的标记，不是搜索关键词本身——请根据消息内容自行提炼检索词。`
    ...
    const stableSystemText = buildStableSystemText(systemPrompt)
    ...
    // BR-2: 主动上下文压缩触发阈值（上一轮上下文 token ≥ 此值即在轮边界摘要中段）。默认 0 = 关闭。
    const compactionTriggerRaw = await window.electronAPI.getSetting('compaction_trigger_input_tokens')
    const parsedCompactionTrigger = Number.parseInt(compactionTriggerRaw ?? '', 10)
    const compactionTriggerTokens = Number.isFinite(parsedCompactionTrigger) && parsedCompactionTrigger > 0 ? parsedCompactionTrigger : 0
    let lastRoundContextTokens = 0
        ...
        llm.chat(
          ...
          (_fullText, toolCalls, reasoning, usage) => {
            ...
            if (usage) {
              ...
              // BR-2: 记录本轮上下文规模作为压缩触发信号。取 input + cache_read + cache_creation 之和：
              // Claude 的 input_tokens 不含缓存，取和才不会在缓存生效时低估真实上下文（宁可略早触发，不可该压不压）。
              lastRoundContextTokens = usage.inputTokens + (usage.cacheReadTokens ?? 0) + (usage.cacheCreationTokens ?? 0)
              safeEmit({
                type: 'usage',
                conversationId,
                timestamp: Date.now(),
    ...
    try {
      ...
      while (pendingToolCalls && pendingToolCalls.length > 0 && round < HARD_MAX_ROUNDS) {
        ...
        // 只是工作上下文，完整对话已双写 SQLite+JSONL，压缩不丢任何持久记录。每轮至多一次、失败降级不压。
        if (
          compactionTriggerTokens > 0 &&
          !compactionDone &&
          lastRoundContextTokens >= compactionTriggerTokens &&
          !isStale()
        ) {
          try {
            const outcome = await compactContextIfSafe({
              messages: apiMessages,
              minTailMessages: COMPACTION_RETAIN_TAIL,
              minMiddleMessages: COMPACTION_MIN_MIDDLE,
              summarize: async (mid) => llm.complete(
                [{
                  role: 'user',
                  content: `以下是较早的多轮对话 / 工具调用历史，请压缩成一段简明摘要，供后续对话继续参考。\n\n**硬性要求**：逐字保留所有关键数字、单位、来源路径 / 文件名、结论；不得编造、不得省略数据来源。只输出摘要正文，不要复述本要求。\n\n=== 历史 ===\n${renderMiddleForSummary(mid)}`,
                }],
                { signal: abortController.signal, temperature: 0 },
              ),
            })
            if (outcome.compacted) {
              apiMessages.splice(0, apiMessages.length, ...outcome.messages)
              compactionDone = true
              logPerf('tool-loop:compact', `round=${round} summarized=${outcome.summarizedCount} ctx=${lastRoundContextTokens}/${compactionTriggerTokens} newLen=${apiMessages.length}`)
            }
          } catch (compactErr) {
            window.electronAPI.logEvent('warn', 'context-compaction-failed', compactErr instanceof Error ? compactErr.message : String(compactErr))
          }
          if (isStale()) return
        }
