...
test('③ 源锚点软警告 hook 经 execute 抓到无 [来源:] 的取数结果，且不阻断回流', async () => {
  ...
  try {
    ...
    // A1 后 knowledge_grep 命中条目自带 [来源: ...#Lx] 锚点 → 正常取数不再触发软警告
    const hit = await router.execute(AVATAR_ID, { name: 'knowledge_grep', arguments: { pattern: '铜' } })
    assert.equal(hit.error, undefined)
    assert.ok(hit.content.includes('[来源: knowledge/a.md#L1]'))
    assert.equal(anchorHook.warnings().length, 0)

    // 零命中结果没有任何锚点可下发 → hook 仍经真实 execute 抓到并记软警告，且不阻断回流
    const miss = await router.execute(AVATAR_ID, { name: 'knowledge_grep', arguments: { pattern: '不存在的词' } })
    assert.equal(miss.error, undefined)
    assert.ok(miss.content.length > 0)
