...
> **更新**: v1.1 新增 Phase 2 知识百科融合实现（Wiki 注入 RAG、答案沉淀、知识演化、交叉引用）
...
│   │   ├── database.ts          ← SQLite 数据库管理（会话/消息/设置）
...
| 日志系统 | `log-event`, `get-activity-logs`, `export-error-log` | 渲染 → 主 |
| 定时自检 | `start-scheduled-test`, `stop-scheduled-test` | 渲染 → 主 |
| 事件推送 | `scheduled-test-trigger`, `test-result-badge` | 主 → 渲染 |
...
| 模块 | 文件 | 职责 |
|------|------|------|
| **DatabaseManager** | `database.ts` | SQLite 数据库管理，含 schema 迁移机制（当前 v2） |
...
| **chatStore** | `chatStore.ts` | Zustand 状态管理：消息列表、对话发送（含 RAG 增强 + 工具调用循环）、记忆自动更新 |
| **TestRunner** | `test-runner.ts` | 测试执行器：调用 LLM 获取回复 → 关键词检查 + AI 评分 |
| **App.tsx** | `App.tsx` | 根组件：分身选择/切换、面板路由、模型配置管理 |
...
数据库内置增量迁移机制（`runMigrations`），通过 `schema_version` 表追踪版本号，每次有结构变更时递增 `CURRENT_SCHEMA_VERSION` 并添加对应迁移逻辑。
...
[渲染进程] chatStore.sendMessage()
  ...
  │     └─ 再次调用 LLM → 回到步骤 3
  │
  ├─ 5. 提取记忆更新标记 [MEMORY_UPDATE]...[/MEMORY_UPDATE]
  │     └─ IPC: write-memory ──▶ 追加写入 MEMORY.md
  │
  └─ 6. 保存消息到数据库
...
│   └── NavButtons               ← [人格|技能|测试|知识库|记忆|设置]
│
├── ChatWindow                   ← 对话窗口
│   ├── MessageList              ← 消息列表
│   │   └── MessageBubble        ← 消息气泡（Markdown 渲染）
│   └── MessageInput             ← 输入框（支持图片粘贴）
│
...
其他状态（会话列表、分身列表、面板开关等）使用 React `useState` 管理，集中在 `App.tsx` 中。
...
| 新增工具函数 | 在 `chatStore.ts` 的 `AVATAR_TOOLS` 添加定义 + `ToolRouter` 添加实现 |
