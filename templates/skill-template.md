<!-- ============================================================
  分身技能标准模版 v1.0
  作者: zhi.qu
  日期: 2026-04-02

  使用说明：
  1. 在分身目录下创建技能文件：avatars/<avatar-id>/skills/<skill-name>.md
  2. 将下方「可复制区」整段粘贴进去
  3. 替换所有 {{PLACEHOLDER}}；删除 HTML 注释与说明文字
  4. 可选：在 skills/ 同目录增加 reference.md、examples.md

  占位符清单：
  - {{SKILL_NAME}}        : 技能文件名，小写+连字符，如 document-image-ingest
  - {{SKILL_TITLE}}       : 技能在正文中的标题
  - {{AVATAR_ID}}         : 所属分身目录名，如 ci-storage-expert
  - {{DESCRIPTION}}       : 技能描述，含 WHAT + WHEN，≤1024 字符
  - {{SKILL_LEVEL}}       : 技能级别，[■] 基础 / [■■] 进阶 / [■■■] 专家
  - {{TRIGGER_SCENARIOS}} : 何时触发本技能（列表）
  - {{TRIGGER_KEYWORDS}}  : 触发关键词列表
  - {{PREREQUISITES}}     : 环境/工具前提（可选）
  - {{WORKFLOW}}          : 分步工作流
  - {{OUTPUT_FORMAT}}     : 输出格式/模板（可选）
  - {{CHECKLIST}}         : 质量检查项（可选）
  - {{ANTI_PATTERNS}}     : 禁止做法（可选）
============================================================ -->

## 可复制区开始（从下一行起复制到技能文件）

```markdown
---
name: {{SKILL_NAME}}
description: {{DESCRIPTION}}
---

# {{SKILL_TITLE}}

> **级别**：{{SKILL_LEVEL}}
> **版本**：v1.0
> **最后更新**：YYYY-MM-DD

---

## 技能说明

[一句话描述这个技能做什么]

## 触发条件

在以下场景应读取并遵循本技能：

{{TRIGGER_SCENARIOS}}

**触发关键词**：{{TRIGGER_KEYWORDS}}

## 输入

用户需要提供：

| 参数 | 是否必须 | 说明 | 示例 |
|------|---------|------|------|
| [参数 1] | 必须 | [说明] | [示例值] |
| [参数 2] | 可选 | [说明] | [示例值] |

如果用户未提供必须参数，主动询问。

## 前提条件（可选）

{{PREREQUISITES}}

## 执行流程

{{WORKFLOW}}

## 输出格式（可选）

{{OUTPUT_FORMAT}}

## 质量检查清单（可选）

{{CHECKLIST}}

## 禁止事项（可选）

{{ANTI_PATTERNS}}

## 引用知识

执行本技能时，优先参考以下知识文件：
- `avatars/{{AVATAR_ID}}/knowledge/文件名.md` — 用途说明

## 示例

### 用户输入
> "[典型的用户输入示例]"

### 分身输出
> "[期望的输出示例（简略版）]"
```

## 可复制区结束

---

## 一、`description` 字段写法（YAML frontmatter）

- **必须用第三人称**（描述会注入系统提示，不说「我」「你可以」）。
- **同时写清 WHAT（做什么）与 WHEN（何时用）**，并包含用户可能说的**触发词**。
- **长度**：非空，建议不超过 1024 字符。
- **name**：仅小写字母、数字、连字符，建议 ≤64 字符。

### 示例（中文为主 + 触发场景）

```yaml
name: pdf-knowledge-ingest
description: >-
  将 PDF 技术文档转为知识库 Markdown，并用 PyMuPDF 渲染关键页为 PNG 后识别图中尺寸与原理图。
  在用户要求把 PDF/DOCX 加入知识库、补充产品手册、或强调「图里的数据也要学」时使用。
```

### 示例（英文）

```yaml
name: security-review-checklist
description: >-
  Reviews changes for OWASP-style risks and secret leakage using a structured checklist.
  Use when reviewing PRs, security-sensitive code, or when the user asks for a security pass.
```

### 触发条件优先写法（强烈推荐）

description 的唯一任务是让模型**在正确的时机加载技能全文**，不是替技能做摘要：

- **以「适用时机」开头**：第一句写"当用户……时使用"（英文技能写 "Use when..."），把触发场景放在最前面。
- **只写触发场景，禁止摘要技能流程**：一旦 description 里出现"先做 A 再做 B"式的流程概要，模型可能读完摘要就以为自己会了，**不再加载技能全文**——技能正文里的红线、反模式、自检清单全部失效。
- **触发关键词穷举**：把用户可能说出的词全部列进去——同义词、中英文、文件扩展名、口语化说法。弱模型（DeepSeek / Qwen）比 Claude 更容易漏触发（undertrigger），关键词每少一个，召回就少一分。

#### 一好一坏对照

```yaml
# ✅ 好：适用时机开头 + 只写触发场景 + 关键词穷举（同义词/中英文/扩展名/口语说法）
name: excel-traceable-query
description: >-
  当用户询问 Excel / CSV / xlsx / 表格 / sheet 中的具体数值、行级数据、
  通过率、不良率、排名、对比，或要求"标到原始 sheet"、"数据要可溯源"、
  "查一下表里"时使用。也适用于用户上传 .xlsx / .csv 文件后提出的任何数值问题。

# ❌ 坏：摘要了技能流程——模型读完摘要以为自己会了，不加载全文，红线全部失效
name: excel-traceable-query
description: >-
  先用 query_excel 的 schema 模式拿列结构，再用 filter 查行级数据，
  最后在每个数字后标注 文件#sheet 来源。
```

---

## 二、推荐目录结构

```
avatars/<avatar-id>/skills/
├── <skill-name>.md        # 必填：核心指令（建议 <500 行）
├── reference.md           # 可选：API、字段说明、长表
├── examples.md            # 可选：输入输出样例
└── scripts/               # 可选：可执行脚本（迁移、校验等）
```

### 目录型技能（复杂技能的推荐形态）

对齐社区事实标准（agentskills.io），有长参考、脚本或静态资源的复杂技能推荐用**目录**组织：

```
avatars/<avatar-id>/skills/
└── <skill-name>/            # 目录名 = frontmatter 的 name（小写 + 连字符）
    ├── SKILL.md             # 必填：核心指令，正文 <500 行
    ├── references/          # 可选：重型参考，按域拆分
    │   ├── <域A>.md         # 如 api-fields.md、excel-schema.md
    │   └── <域B>.md         # >300 行的 reference 必须在文件开头带目录
    ├── scripts/             # 可选：可执行脚本
    └── assets/              # 可选：图片、文档模板等静态资源
```

**硬性约定**：

- frontmatter 的 `name` 必须与目录名一致（kebab-case：小写字母 + 数字 + 连字符）。
- `SKILL.md` 正文控制在 500 行以内；放不下的内容拆进 `references/<域>.md`。
- **引用限一层深**：SKILL.md 可以引用 `references/` 下的文件，但 reference 之间不得再互相引用（模型按需二次读取时只跟一层链接）。
- 单个 reference 超过 300 行时，文件开头必须带目录（标题清单），方便模型按节跳读。

**与单文件格式的兼容关系**：现有单文件技能（`<skill-name>.md`）**继续有效**，无需迁移；目录型是复杂技能的推荐形态。同名覆写仍按 local > shared > community 优先级处理，与文件/目录形态无关。

**分身专属技能**（随分身创建）：`avatars/<avatar-id>/skills/<skill-name>.md`
**公共共享技能**（已建立）：`shared/skills/<skill-name>.md` — 跨分身通用技能的唯一正本

### 技能路径约定

| 类型 | 路径 | skill-index.yaml 中的 source 字段 | 优先级 |
|------|------|------|------|
| 分身专属 | `avatars/<id>/skills/<name>.md` | `source: local` | 最高 |
| 公共技能 | `shared/skills/<name>.md` | `source: shared` | 中 |
| 社区开源 | `shared/skills/community/<pkg>/skills/<name>.md` | `source: community` | 最低 |
| 本地覆写 | `avatars/<id>/skills/<name>.md` | `source: local` + `overrides: shared/skills/<name>.md` | 最高 |

**覆写机制**：当分身需要在公共技能基础上增加领域特定内容（如工商储专用示例、空数据兜底模板等），在本地 `skills/` 创建同名文件作为覆写版本，并在 `skill-index.yaml` 中标注 `overrides` 字段。

**社区技能**：来自 GitHub 的开源技能，通过 `shared/skills/sources.yaml` 声明来源，运行 `scripts/soul-sync.sh` 安装到 `community/` 目录。发布者需在仓库根目录放置 `skill-manifest.yaml`（参考 `templates/skill-manifest-template.yaml`）。

---

## 三、`reference.md` 模版片段（可选新建）

```markdown
# {{SKILL_TITLE}} — 参考说明

## 术语表

| 术语 | 含义 |
|------|------|

## 工具与命令速查

| 场景 | 命令 / 库 |
|------|-----------|

## 与分身其它组件的配合

- 分身定义：`avatars/<avatar-id>/soul.md`
- 分身知识库：`avatars/<avatar-id>/knowledge/`
- 分身记忆：`avatars/<avatar-id>/memory/MEMORY.md`
```

---

## 四、`examples.md` 模版片段（可选新建）

```markdown
# {{SKILL_TITLE}} — 示例

## 示例 1：用户输入

（摘录典型用户一句话或任务描述）

## 示例 1：期望行为

（分身应执行的步骤摘要或输出结构）

## 示例 2：…
```

---

## 五、判断力规则化三节（taste-skill 写法，红线/交付类技能强烈推荐）

品味和判断力能被规则化的核心机制是：**识别并封杀模型的统计均值输出**。在技能正文中增加以下三节（可直接抄改）：

### 5.1 `## 反模式（禁令 + 归因 + 后果）`

每条反模式必须写满三段：**禁什么 + 为什么禁（实测事故 / AI 惯性，点名到具体值）+ 违反后果**。只写"禁止 X"不写归因，模型会在压力下自我说服绕过禁令。

```markdown
## 反模式（禁令 + 归因 + 后果）

| 禁令 | 归因（为什么禁） | 违反后果 |
|------|----------------|---------|
| 禁止用 `knowledge/*.md` 二手总结冒充"原始 sheet"来源 | 实测事故（2026-05-22）：用户要求"每个数字标到原始 sheet"，分身把所有来源标成 `knowledge/*.md`，没调 `query_excel`，被判定"来源错位" | 用户对整段数据失去信任，回答作废重做 |
| 禁止在拒答中复述题面单位 / 型号 / 友商名 | AI 惯性：复述显得"专业"，但等于给编造留了锚点 | 触发 L9 红线，一票否决 0 分 |
| [禁止 XXX] | [事故日期 + 现象，或 AI 惯性的具体表现，点名到具体值] | [具体后果] |
```

### 5.2 `## 交付前自检`

条目**尽量可机械判定**——能打勾/打叉的写法，不写"数据要可溯源"这种无法判定的原则句。与 `tests/cases/` 互补：测试是外部验收，自检是技能内嵌门禁。

```markdown
## 交付前自检

- ☐ 每个数字能定位到 `文件#sheet` 级（不是"来源：知识库"）
- ☐ 每个 `[来源: ...]` 只覆盖一个数字/一句结论，没有一行来源盖整段
- ☐ 输出里没有"据统计 / 一般来说 / 业界通常"字样
- ☐ [可机械判定的检查项 N]
```

### 5.3 `## 不适用范围（转介给谁）`

明确技能**不做什么**、被误路由时转介到哪，防止技能被错误触发后硬着头皮输出。

```markdown
## 不适用范围（转介给谁）

- 不做 [场景 X]（如：不做 dashboard 布局设计）：转介 `shared/skills/<技能名>`
- 不做 [场景 Y]（如：跨区域政策解读）：超出本分身领域，按拒答模板引导用户找 [某分身]
```

---

## 六、与「带知识库的分身」配合时的建议

若技能服务于某分身的 `knowledge/` 目录：

1. 在技能文件中写明**知识库根路径**与**允许的文件类型**（如仅 Markdown + `images/`）。
2. 文档入库流程中**单独一节**写清：**文本提取**与**图片页渲染识别**不得省略（与分身定义中的「文档图片识别」一致时可引用该分身说明）。
3. 产出物命名约定（如 `产品名-主题.md`）写在一处，避免重复造文件名规则。

---

## 七、自检清单（发布技能前）

- [ ] `name` 符合字符集且与文件名一致（目录型技能：与目录名一致）
- [ ] `description` 以「适用时机」开头，只写触发场景，未摘要技能流程，第三人称
- [ ] 触发关键词已穷举（同义词、中英文、文件扩展名、口语说法）
- [ ] 目录型技能：SKILL.md 正文 <500 行，reference 引用只有一层，>300 行的 reference 带目录
- [ ] 红线/交付类技能：已包含「反模式」「交付前自检」「不适用范围」三节
- [ ] 技能级别已标注（[■] 基础 / [■■] 进阶 / [■■■] 专家）
- [ ] 输入参数表已列出必须和可选参数
- [ ] 触发关键词已明确列出
- [ ] 正文无与通用模型常识重复的长篇解释（保持精简）
- [ ] 长内容已拆到 `reference.md`，技能文件内仅一层链接
- [ ] 知识库路径指向正确的 `avatars/<avatar-id>/knowledge/`
- [ ] 包含至少一个用户输入/分身输出示例
