...
[![Release](https://img.shields.io/badge/release-v0.6.4-green.svg)](CHANGELOG.md)
...
- **批量导入 + 自动优化** — 文件夹 / zip / tar.gz / 7z / rar 一次性导入，原始文件保留到 `_raw/`，导入后 ENHANCE 走完整管线（OCR → 清洗 → LLM 格式化 → 数值校验 → 索引重建），支持断点续跑
- **图表可视化** — 内置 `draw-chart` 技能基于知识库数据生成 ECharts 图表，UED 高级审美（渐变面积填充、毛玻璃 tooltip、圆角柱状图、smooth 折线、5 色 palette、aria 色盲友好）；**数据守护规则**：< 3 数据点禁用折线图、markLine/markPoint 数据点门槛、稀疏数据诚实告知、禁止 emoji legend icon
...
- **跨平台打包自动 rebuild** — `npm run dist:win` / `dist:linux` / `dist:all` 完成后自动 `npx @electron/rebuild -f -w better-sqlite3` 恢复本机 native binding，避免 Windows DLL 污染 Mac 开发环境
