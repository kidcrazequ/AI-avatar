...
### 4.14 P3 WebDAV 跨设备同步（技术方案 · 2026-05-09）

> **状态（2026-05-09）**：由窗口 **指挥官-W19-#16-2026-05-09** 按 §10.5 落盘。
> **决策核心**：**走 Cherry Studio 全量 zip 快照路线**（M 工作量可达），**不**走 Remotely-Save 真正双向同步路线（实测 v3 算法即使有"删除日志"仍有 Issue #991 / #708 / #677 三类已知 bug，需要 6-9 个月迭代成熟，超出本期 M 预算）。MVP 解决"用户换电脑了把分身搬过去"的核心痛点；真正双向同步如未来需要单独立项 §4.14.A。
> **路径勘误**：启动卡序 7 写「`packages/core/src/sync/webdav-*`」，但 fs / 网络 / safeStorage 均在主进程，**实际**主进程逻辑在 **`desktop-app/electron/sync/`**；纯函数（manifest 序列化 / 路径白名单 / 增量算法）放 `packages/core/src/sync/`。
> **热点文件**：`desktop-app/electron/sync/`（新建目录）+ `desktop-app/electron/database.ts`（v11→**v12** 迁移 + `sync_history` 表）+ `desktop-app/electron/main.ts`（启动 sync-manager + 7 IPC）+ `desktop-app/electron/preload.ts`（sync* invoke 包装）+ `desktop-app/src/components/SettingsPanel.tsx`（TOOLS Tab 内「跨设备同步」子区，约 280 行）+ `desktop-app/docs/webdav-sync.md`（新，与 `web-embed-widget.md` / `langbot-integration.md` 同 lane）。**严禁**改 `desktop-app/electron/proxy-server.ts`（保 §4.7）；**严禁**改 `packages/core/src/knowledge-retriever.ts`（保 §4.12）。
...
| 项 | 现状 | 影响本方案 |
|---|---|---|
| 数据持久化分布 | `userData/xiaodu.db` + `userData/conversations/<id>.jsonl` + `userData/avatars/<id>/`（生产，`main.ts:65-77`）+ 仓库 `avatars/<id>/`（开发） + `shared/` + `userData/logs/` + `userData/attachments/` + `userData/tool-results/` + `userData/backups/` | 必须区分"用户资产"（同步） vs "本地缓存/日志/临时"（不同步） |
| SQLite 模式 | better-sqlite3 默认 WAL；live 文件 `xiaodu.db` + `xiaodu.db-wal` + `xiaodu.db-shm` 强一致 | **不能**直接 `fs.copyFile` 上传 `.db`，必须用 `db.backup()` API 导出静态快照（`database.ts:1049-1057` 已有） |
| 凭据加密 | `proxy_api_token` / Tavily 等存 settings 表**明文**（`database.ts:920-927` + `SettingsPanel.tsx:792`）；GitHub PAT 经 `safeStorage.encryptString` 加密（`github-connector.ts:69-72`） | WebDAV 密码必须走 `safeStorage`（与 PAT 同模式），**不**沿用 settings 明文 |
| schema_version | #15 已升至 v11（`database.ts:6-7`） | 本方案需 v11→**v12** + 新表 `sync_history` |
| SettingsPanel | 2627 行 / 10 Tabs，`TOOLS` Tab 内已有 Proxy / MCP / Web Embed 子区，#15 子区 290 行 | 同步子区加在 TOOLS Tab 内（不增顶级 Tab），与 #15 同款风格 |
| 备份基础设施 | `database.ts:1049-1057` `async backup(dest)` 已实现；`main.ts:4437-4467` `db-backup` IPC + 保留 7 份本地备份 | **可复用**作为 SQLite 静态快照导出步骤；本方案在云端做"快照 zip"包含此 backup 产物 |
| Cron 调度器 | `cron-scheduler.ts` 已支持 `scheduleCron(taskId, cronExpr, timezone, callback)`（#11 落盘） | **可复用**作为自动间隔同步触发器（taskId='webdav-sync'） |
| Logger 体系 | `Logger` 类 + `logEvent('warn', ...)` + 文件轮转 5MB | 本方案审计日志走 `logEvent('webdav-sync', ...)` |
| 现有 npm 依赖 | **未**装 `webdav` / `archiver` / `keytar` 等同步相关库 | 本方案需新增 `webdav@^5.9.0`；不引 keytar（用 safeStorage） |
| 跨进程边界 | 渲染端 `chatStore` / SettingsPanel 通过 IPC 调主进程 | sync 必须**全部在主进程**（fs / 网络 / safeStorage / SQLite backup） |

##### 业界关键参考（详见 `.cursor/plans/对手对比融合-W19-#16-webdav-research.md`）

| 维度 | Cherry Studio | Remotely-Save v3 | Joplin | Soul 选择 |
|---|---|---|---|---|
| 同步粒度 | **全量 zip 快照**（按 device + 时间戳） | 真正双向（每文件 mtime+size+hash） | 文件级 + delta sync | **全量 zip 快照**（MVP 简单可靠） |
| 触发频率 | 手动 + 自动间隔 + IPC | 用户主动 + 启动 + 定时 | 启动 + 间隔 | 手动 + 自动间隔（关闭/每天/每 6 小时/每小时） |
| 冲突策略 | **无双向 merge**，用户选哪份快照 | LWW + keep both（PRO） | Conflict notebook | **MVP 无双向 merge**，明确单向（备份 / 恢复二选一） |
| 凭据存储 | safeStorage（PR #11934） | OS keyring | 用户主密码派生 | **safeStorage**（与 PAT 同模式） |
| WebDAV client | **`webdav@5.x`**（perry-mitchell） | `webdav@5.x` | 自实现 | **`webdav@^5.9.0`**（事实标准） |
| E2EE | AES IPC（备份内容侧） | v3 新加密 | 两层 master key | **MVP 不内置 E2EE**（推荐 HTTPS + 服务器信任，未来升级单立项） |
...
1. **同步范围（明确白名单）**
   ```text
   ✅ 必同步（用户资产）：
   - userData/avatars/<id>/         （分身配置 + 知识库 + memory + structured memory + skills + tests + projects）
   - shared/                        （共享技能、知识、设计语料；soul-sync.sh 拉的 community/ 也含）
   - userData/conversations/<id>.jsonl  （对话冗余备份，与 SQLite 双写）
   - SQLite 快照：xiaodu-snapshot-<timestamp>.db  （用 db.backup() 导出，**不**上传 live .db / -wal / -shm）

   ❌ 绝不同步（本地状态 / 缓存 / 临时）：
   - userData/xiaodu.db / xiaodu.db-wal / xiaodu.db-shm  （live SQLite，跨网络共享会损坏）
   - userData/logs/                 （审计日志，本设备相关 + 体积大）
   - userData/attachments/          （附件二进制，单文件可达 50MB；MVP 不同步，未来按需）
   - userData/tool-results/         （工具结果 spool，临时落盘）
   - userData/backups/              （本地数据库备份轮转，与云端备份冗余）
   - avatars/<id>/_cache/           （图表缓存，可重建）
   - avatars/<id>/knowledge/_index/ （embedding 索引缓存，可重建；MVP 不同步以缩小快照体积；恢复后用户重建）
   - avatars/<id>/knowledge/_excel/ （Excel 中间 JSON；MVP **同步**因为是 RAG 必需，不可重建）
   ```
   **MVP 不同步 attachments/ 与 _index/ 的理由**：① attachments 单文件 50MB 上限，单备份可能轻松超 1GB 远超坚果云 500MB 限制；② _index 是 embedding 缓存，恢复设备重建即可（也避免跨平台 binary 不兼容）。**未来如需扩展可在子任务 2 的 `WHITELIST` 常量加项**，不需改架构。

2. **快照打包格式**
   ```text
   soul-backup-<deviceId>-<YYYY-MM-DD>-<HH-mm-ss>.zip  （命名风格借鉴 Cherry Studio）
   ├── manifest.json                  （sha256 + 文件清单 + Soul 版本 + 设备信息 + schemaVersion）
   ├── snapshot/
   │   ├── xiaodu-snapshot.db         （由 db.backup() 导出的静态快照）
   │   ├── avatars/<id>/...           （userData/avatars/）
   │   ├── shared/...
...
3. **WebDAV 远端目录约定**
   ```text
   /<webdav-root>/<user-configured-path>/    （用户配置，默认 /soul-backup/）
   ├── soul-backup-<device>-2026-05-09-09-00-00.zip
   ├── soul-backup-<device>-2026-05-09-15-00-00.zip
   └── ...（保留最近 N 份，可配置 1-30，默认 7）
   ```

4. **核心架构（双进程边界）**
   ```text
   渲染进程（SettingsPanel.tsx）
     └─ 调 IPC sync:* → 主进程 sync-manager.ts
                        ├─ credential-store.ts （safeStorage 加密 / 解密）
                        ├─ snapshot-builder.ts （SQLite backup + zip 打包 + sha256 manifest）
                        ├─ webdav-client.ts    （perry-mitchell webdav 包，PUT / PROPFIND / DELETE）
                        ├─ db-sync-history.ts  （记录每次同步结果到 SQLite）
                        └─ cron-scheduler.scheduleCron('webdav-sync', cronExpr, ...)（自动触发）
   ```

5. **凭据加密（与 GitHub PAT 同款）**
   - 字段：`webdav_password_encrypted`（settings 表存 base64 后的 safeStorage 密文）
   - 服务端 / 用户名 / 路径 / 启用开关 / 自动间隔等非敏感字段存 settings 表明文（`webdav_endpoint` / `webdav_username` / `webdav_path` / `webdav_enabled` / `webdav_auto_interval`）
   - **Linux safeStorage 兜底**：调用 `safeStorage.getSelectedStorageBackend()`，若返回 `basic_text` 则 UI 弹窗"无可用 keyring，密码将仅 base64 编码（不安全），是否继续？"，需用户主动确认才能保存

6. **不内置 E2EE（MVP 范围）**
   - 决策：MVP 文档明确"**WebDAV 服务器必须是用户信任的**，请用 HTTPS"
   - 理由：① 全量 E2EE 是 L 工作量（参考 Joplin 两层 master key 实现）超 M 预算；② Soul 数据敏感性低于聊天历史 + 财务报表（可由用户选择信任坚果云 / Synology / 自建 Nextcloud）
   - **未来升级路径**预留：snapshot-builder 内 `encryptZip(zipPath, password): Buffer` 抽象点（MVP 实现为直通），未来可加 AES-GCM 整体加密层，单立项

7. **大文件切分策略（MVP 简化）**
   - 单 zip ≤ 500 MB（坚果云上限）
   - **MVP 不做自动切分**：超过 500 MB 上传前在 UI 报错，引导用户清理 attachments / 重置 _excel 等
   - **未来扩展点**：snapshot-builder.ts 内 `splitZip(zipPath, maxBytes)` 抽象（MVP 不实现），单立项

8. **频率与触发**
   - **手动**：设置面板 "立即备份" / "从备份恢复" 按钮（最常用）
   - **自动**：可配置（关闭 / 每天 / 每 6 小时 / 每小时），用 `cron-scheduler.scheduleCron('webdav-sync', cronExpr)` 触发；与 #11 schedules 共享 cron 调度器（不重复造）
   - **不**在应用启动时自动同步（避免开机阻塞）
   - **不**在退出时强制同步（避免延迟关闭）
   - **不**用 fs.watch 触发实时同步（避免 mtime 抖动 + 文件锁竞态；与 chart-cache.ts 现有"不做 watch"决策一致）

9. **冲突策略（MVP 单向操作）**
   - "立即备份" → 上传新 zip，覆盖远端同名（按 timestamp 命名，自然不冲突）
   - "从备份恢复" → 用户必选远端某份 zip → **本地先做兜底备份**（`local-pre-restore-<timestamp>/` 含 SQLite + avatars zip）→ 解压远端 zip 替换本地 → 重启应用
   - **MVP 无双向 merge**：不实现"两边都改了怎么办"的算法；用户必须明确选 push 或 pull
   - **多设备共存提示**：UI 显示 "上次备份设备: <deviceA>"，若当前设备 ≠ <deviceA> 弹窗"远端备份来自其他设备，恢复将覆盖本地数据，确认？"

10. **可观测性 + 审计**
    - settings 表：`webdav_last_sync_at` / `webdav_last_sync_status[success|failed|in_progress]` / `webdav_last_sync_error`（错误摘要）
    - 新表 `sync_history`：保留最近 30 条（id / direction[backup|restore] / status / file_count / total_bytes / duration_ms / remote_filename / error_message / created_at）
    - 设置面板"上次同步"卡片显示 + "查看历史"按钮（弹窗列出 sync_history）
    - Logger.logEvent 审计：`{ event: 'webdav-sync', direction, status, duration_ms, remote_filename }`，与现有 tool-calls JSONL 同 lane

11. **路径标准化（生产 vs 开发）**
    - 生产：`userData/avatars/`（`main.ts:resolveAvatarsPath()`）
    - 开发：仓库 `avatars/`（`main.ts:65-77`）
    - **本方案统一以 `resolveAvatarsPath()` 返回值为同步源**（开发模式下也走仓库 avatars/）
    - 恢复时若目标 `avatars/` 已存在 → 弹窗确认"将覆盖本地分身，是否继续？"

12. **坚果云 quirks 适配（中国用户优先）**
    - PROPFIND 用 `Depth: 1`（不依赖 `Depth: infinity`）
    - 单 PROPFIND ≤ 750 项（备份目录通常 < 30 个 zip，不会触及）
    - 30 分钟 600 次 → MVP 单次备份 = 1 PUT + 1 PROPFIND，远低阈值
    - 自动重试：429 / 5xx 时指数退避（1s → 2s → 4s 共 3 次）
    - 自签证书：UI 提供"忽略 HTTPS 证书校验"勾选（默认关闭，与 Cherry Studio 同款）

13. **DB schema v11→v12**
    ```sql
    CREATE TABLE IF NOT EXISTS sync_history (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      direction TEXT NOT NULL,                  -- 'backup' | 'restore'
      status TEXT NOT NULL,                     -- 'success' | 'failed' | 'in_progress'
      file_count INTEGER NOT NULL DEFAULT 0,
      total_bytes INTEGER NOT NULL DEFAULT 0,
      duration_ms INTEGER NOT NULL DEFAULT 0,
      remote_filename TEXT,                     -- 如 soul-backup-deviceA-2026-05-09-09-00-00.zip
      error_message TEXT,                       -- 失败时记录
      created_at INTEGER NOT NULL
    );
    ...
    ```
    与 `embeds`（v11）同款 transaction 包裹 + `safeAddColumn` 模式。

14. **device id 生成**
    - 首次启动生成 `device_id`（UUID v4）存 settings 表
    - 用于 zip 命名 + manifest.json + UI 显示"备份来自设备 <deviceId>"
    - Soul 已在 settings 表存机器相关 ID 的先例（参考 main.ts deviceId 处理），**优先复用**；无则新建

#### 任务拆分（实施用 · 待用户确认后执行）

| 子任务 | 涉及文件 | 工作量 | 说明 |
|---|---|---|---|
| 1. WebDAV client + 凭据存储 + 设置 | `desktop-app/electron/sync/webdav-client.ts`（新，约 200 行）+ `desktop-app/electron/sync/credential-store.ts`（新，约 120 行）+ `desktop-app/package.json`（+`webdav@^5.9.0`） | M（300-350 行 + 1 依赖） | 封装 perry-mitchell webdav 包；safeStorage 加密 / 解密；getSelectedStorageBackend 兜底；7 个公开方法（connect / putBackup / listBackups / getBackup / deleteBackup / testConnection / disconnect） |
| 2. 快照打包 + manifest 序列化 + 数据范围白名单 | `desktop-app/electron/sync/snapshot-builder.ts`（新，约 250 行）+ `packages/core/src/sync/snapshot-manifest.ts`（新纯函数，约 80 行）+ `desktop-app/package.json`（+ `archiver`<sup>1</sup>） | M（300-350 行 + 1 依赖） | DB.backup() → static .db；archiver 流式打 zip；manifest.json sha256 + Soul 版本 + 设备信息；WHITELIST 常量列同步范围；500MB 上限校验抛错 |
| 3. DB v11→v12 迁移 + sync_history DAO | `desktop-app/electron/database.ts`（+CURRENT_VERSION=12 + createBaseSchema 追加 + v11→v12 迁移块）+ `desktop-app/electron/db-sync-history.ts`（新，约 200 行 SyncHistoryStore + 单测） | S（250-300 行） | 与 #15 embeds 同款 transaction + safeAddColumn 模式；DAO 单测 ≥ 8 用例（CRUD + 限 30 条最旧淘汰 + 查询） |
| 4. 主进程 sync-manager + IPC + cron 复用 + 启动恢复 | `desktop-app/electron/sync/sync-manager.ts`（新，约 350 行）+ `desktop-app/electron/main.ts`（+ syncManager lazy 单例 + app.whenReady 启动注册自动间隔 + 7 IPC：`sync:get-config / set-config / test-connection / backup-now / restore-from / list-history / clear-credentials`）+ `desktop-app/electron/preload.ts`（+ 7 个 sync* invoke） | M（500-600 行） | runBackup() 编排：snapshot-builder → webdav-client.putBackup → sync-history.record；runRestore() 反向：getBackup → 先本地 pre-restore zip → 解压 → app.relaunch()；cron 复用 #11 scheduleCron('webdav-sync', expr) |
| 5. 设置面板 UI（同步子区） | `desktop-app/src/components/SettingsPanel.tsx`（TOOLS Tab 内扩展约 280 行）+ `desktop-app/src/global.d.ts`（+ SyncConfig / SyncHistoryRow 类型 + electronAPI 7 方法签名） | M（350-400 行） | 服务器配置（endpoint / username / password / path）+ 测试连接按钮 + 启停开关 + 自动间隔下拉 + 立即备份 / 从备份恢复按钮 + 历史日志弹窗 + safeStorage basic_text 警告 + Linux Flatpak 提示 |
| 6. 测试 + 文档 + 端到端冒烟 | `desktop-app/electron/sync/__tests__/`（webdav-client.test.ts / snapshot-builder.test.ts / sync-manager.test.ts）+ `desktop-app/electron/db-sync-history.test.ts` + `packages/core/src/tests/snapshot-manifest.test.ts` + `desktop-app/docs/webdav-sync.md`（新，约 350 行：5 节大纲 + 9 条 FAQ + 坚果云 / Nextcloud / Synology 各一份配置示例） | M（450-550 行测试 + 文档） | 至少 18 用例：credential 加密往返 / manifest sha256 一致 / WHITELIST 边界 / sync_history CRUD + 30 条淘汰 / sync-manager backup 全链路 mock / restore 前自动 pre-restore；端到端冒烟用免费坚果云账号实测一次 backup + restore |

> **<sup>1</sup> archiver 依赖**：单独考虑过用 Node 内建 zlib 手写 zip，但 zip 格式 spec 复杂（local file headers / central directory / end of central directory record），手写易错；archiver 是 Node 生态事实标准（npm 周下载 800K+），M 工作量内不应自造轮子。

> **不做**子任务（明确排除，避免范围蔓延）：
> - ❌ 真正双向同步算法（本期 MVP 单向操作；如需要单立项 §4.14.A，参考 Remotely-Save v3）
> - ❌ E2EE（本期 MVP 推荐 HTTPS；如需要单立项 §4.14.B，参考 Joplin 两层 master key）
> - ❌ 大文件自动切分（本期 500MB 上限报错；如需要单立项 §4.14.C，与 Nextcloud chunked upload v2 对齐）
> - ❌ attachments/ 同步（本期 WHITELIST 排除；用户如有需求单立项）
> - ❌ embedding 索引 `_index/` 同步（本期排除，恢复后重建；用户如有需求单立项）
> - ❌ 应用启动时同步 / 退出时强制同步（避免阻塞 UX）
> - ❌ fs.watch 实时同步（避免 mtime 抖动；与 chart-cache.ts 现有决策一致）
> - ❌ 多账号 WebDAV 同时同步（本期单账号；UI 不留多账号入口）

#### 风险

- ⚠️ **跨设备 device_id 冲突**：同一份云端目录可能被多设备备份/恢复混用，UI 必须明确"恢复的是设备 X 的备份"，并强制本地兜底备份（`local-pre-restore-<timestamp>/`）防止误覆盖
- ⚠️ **safeStorage Linux basic_text 风险**：getSelectedStorageBackend 必须前置检查 + UI 警告；用户主动确认后仍以 base64 存（不阻塞功能，但明确"不安全"）
- ⚠️ **大体积分身**：知识库可达 GB 级，单 zip 易超 500 MB；UI 在打包前预估体积，超限报错引导用户排除 attachments / 重置 _excel
- ⚠️ **自动间隔与 #11 schedules 共享 cron 但优先级低**：webdav-sync 任务每次触发可能耗时数十秒（zip 打包 + 上传），若与用户 schedule 同时触发应排队；cron-scheduler 默认 fire-and-forget，本期接受偶尔并发（archiver / webdav-client 都是流式，不互锁），未来如需严格互斥单立项
- ⚠️ **跨 OS 路径**：Windows backslash vs Unix slash，archiver 默认用 Unix slash；恢复到 Windows 设备时验证路径正常
- ⚠️ **HTTPS 自签证书**：Cherry Studio Issue #11934 + #2522 均提到 Electron 主进程 https.Agent + rejectUnauthorized 处理，本方案沿用（设置项 `webdav_ignore_tls_errors` 默认 false）
- ⚠️ **app.relaunch() 后状态**：恢复完成必须 relaunch 让 SQLite 重新打开新文件 + chatStore 重新加载（与 Cherry Studio 一致）；用户教育"恢复后应用会自动重启"

#### 完成标准

- 用户在 Soul 设置面板 → TOOLS Tab → 跨设备同步子区配置坚果云 / Nextcloud / Synology 任一 → 测试连接成功 → 点"立即备份"，远端目录有 `soul-backup-<device>-<timestamp>.zip` 文件
- 在另一台设备相同设置 → 看到远端备份列表 → 选最新 zip → 点"从备份恢复" → 自动本地兜底备份 → 用户确认 → 应用重启 → 分身、知识库、对话历史完整恢复
- 凭据用 safeStorage 加密；Linux 无 keyring 时 UI 弹窗警告；Flatpak 提示走 portal.Secret
- 自动间隔（关闭 / 每天 / 每 6 小时 / 每小时）按配置经 cron-scheduler 触发；sync_history 表记录每次结果
- 单测覆盖 ≥ 18 用例；tsc 双侧零错误；改动**不触碰** `proxy-server.ts` / `knowledge-retriever.ts` / `tool-router.ts`（git diff 为空）
- 文档 `desktop-app/docs/webdav-sync.md` 让外部新同事 30 分钟内可完成「配置坚果云 → 备份 → 在另一设备恢复」全流程
- 主计划 §2 「P3 WebDAV 跨设备同步」从 ⏳ → ✅；进度看板回写关键决策 ≥ 5 条 + 下游解锁 + 文件热点释放

---

### 4.15 P2 流式语音输入（豆包 ASR · 大模型流式 · 技术方案 · 2026-05-10）

> **状态（2026-05-10）**：由窗口 **指挥官-W20-#12-2026-05-10** 按 §10.5 落盘：explore 摸清 Soul 代码现状 + docs-researcher 对齐火山引擎官方文档。  
> **边界**：首期只做 **Soul 内对话输入框** 的「按住说话 → 流式出字」；**不做**系统级全局语音输入、不做与 `proxy-server` / `knowledge-retriever` 并行改造；**不弱化** §5 已确认的 OCR/RAG/引用 UI（本任务独立链路）。

#### 目标

- 用户在 **`MessageInput`（`desktop-app/src/components/MessageInput.tsx`）** 侧可启用流式语音识别，边说边将识别文本 **增量写入** 输入框（可编辑、可发送前修改）。
- 识别引擎采用火山引擎 **豆包语音 · 语音识别大模型 · 大模型流式语音识别 API**（WebSocket + 二进制协议），与主计划 §2「Proma / 豆包 ASR」价值对齐。
- 密钥与资源 ID 走 **SQLite `settings` 表** + 现有 `get-setting` / `set-setting` IPC + **`SettingsPanel` 表单项** 模式（与 `*_api_key` 同类），**禁止**把 `X-Api-Key` 写进渲染进程可读的明文配置文件或提交到 git。

#### 现状摘要（explore · 2026-05-10）

| 维度 | 要点 |
|------|------|
| 输入 UI | `MessageInput.tsx`（textarea + 附件 + 发送）；`ChatWindow.tsx` 底部挂载 |
| 既有音频 | **无** `MediaRecorder` / `getUserMedia` / 既有 WebSocket ASR 客户端 —— **绿field** |
| 配置落盘 | `database.ts` `settings` 表 + `main.ts` `get-setting`/`set-setting` + `preload` + `SettingsPanel` 批量 `setSetting` |
| 渲染进程 HTTP | `@soul/core/browser` 导出 `fetchWithTimeout`；`llm-service.ts` 对 LLM 流式用原生 `fetch`+`ReadableStream`（因需读非 2xx body） |
| 主进程 | 已有 `fetchWithTimeout`；**无**现成 `WebSocket` 客户端业务代码 |
| 权限 | `main.ts` 创建 BrowserWindow **未**注册 `session.setPermissionRequestHandler`；增加麦克风前需补齐 **Chromium 权限弹窗 / 拒绝策略**（macOS/Windows 行为差异） |

#### 官方文档与 API 要点（docs-researcher · 摘要）

| 项 | 内容 |
|----|------|
| **主文档** | [大模型流式语音识别 API](https://www.volcengine.com/docs/6561/1354869)（WebSocket、`wss://`，binary 帧：header + 大端 length + payload；可 Gzip） |
| **端点** | 双向流式 `wss://openspeech.bytedance.com/api/v3/sauc/bigmodel`；流式输入 nostream `wss://openspeech.bytedance.com/api/v3/sauc/bigmodel_nostream` |
| **鉴权（握手 HTTP Header）** | 新版推荐 **`X-Api-Key`** + **`X-Api-Resource-Id`**（如 `volc.bigasr.sauc.duration` / `volc.bigasr.sauc.concurrent` 或 2.0 `volc.seedasr.sauc.*`，**以控制台开通为准**）+ `X-Api-Request-Id`（UUID）+ `X-Api-Sequence: -1`；旧版 `X-Api-App-Key` + `X-Api-Access-Key` |
| **音频** | `format`/`codec`/`rate`/`bits`/`channel`；**PCM 仅支持 16kHz、16bit**；分包建议 **100–200ms/包**（双向流式推荐约 **200ms**） |
| **计费 / 配额** | [计费概述](https://www.volcengine.com/docs/6561/1359369)、[QPS/并发查询](https://www.volcengine.com/docs/6561/1476626) |
| **产品区分** | 文档树中「流式语音识别」经典商品（`volc.streamingasr.*`）与 **大模型 `sauc` WebSocket（本方案）** 不同资源；Soul 对齐 **`1354869` 大模型流式** |

#### 关键设计决策（草案 · 待实施窗口确认）

1. **连接所在进程**：**主进程（或专用 helper）持有 WebSocket**，渲染进程只通过 **IPC 发送 PCM 分片 + 接收部分转写事件**。理由：浏览器 `WebSocket` **无法设置**豆包要求的 **自定义握手 Header**（`X-Api-Key` 等）；且避免 API Key 进入渲染 bundle。
2. **录音实现**：优先 **渲染进程 `getUserMedia` + AudioWorklet 或 ScriptProcessor 降采样到 16kHz s16le`**，将分片经 IPC 发到主进程转发（与 §8.4「敏感能力走主进程审计面」方向一致）；若遇性能问题再评估 **主进程 desktopCapturer**（非首选）。
3. **依赖**：主进程 WebSocket 使用 Node **`ws`** 或 **`undici`/原生** 以仓库现有栈为准（**新增依赖须记入 `desktop-app/package.json` 并说明体积**）；二进制帧拼装优先 **移植官方示例逻辑**（Python/Go 压缩包）为 TypeScript 模块，避免手写 bit 误差。
4. **与 §5 隔离**：不改 `knowledge-retriever.ts` / `document-parser.ts` / `SourceCitation.tsx` / OCR 管线；ASR 输出仅进入 **用户输入草稿字符串**。

#### 任务拆分（实施用 · 待用户确认后执行）

| 子任务 | 涉及文件（预计） | 工作量 | 说明 |
|--------|------------------|--------|------|
| 1. 协议客户端 + 单测（纯逻辑） | `packages/core/src/audio/` 下 **`volc-bigmodel-asr-protocol.ts`**（或 `desktop-app/electron/asr/` 若坚持零进 core）+ `packages/core/src/tests/volc-bigmodel-asr-protocol.test.ts` | M | 仅实现 **full client request** / **audio-only request** 打包与 **full server response** 解析 + gzip 分支；用 fixture 二进制快照做 golden test；**无**真实网络 |
| 2. 主进程 ASR 会话 + IPC | `desktop-app/electron/asr-session.ts`（新）+ `main.ts`（`asr:start` / `asr:push-pcm` / `asr:stop` / `asr:cancel` + 事件 `asr:partial`）+ `preload.ts` + `global.d.ts` | L | 读 `settings` 中 apiKey/resourceId；建连、心跳/重连策略（文档错误码如 `55000031` 繁忙退避）；单会话互斥 |
| 3. 渲染录音 + MessageInput UI | `MessageInput.tsx` + 小 hook（如 `useMicPcm16k.ts`）| M | 麦克风按钮、录音中状态、错误 Toast；分片 `window.electronAPI.asrPushPcm`；订阅 `asr:partial` 追加到 textarea |
| 4. 设置面板 | `SettingsPanel.tsx` | S | `doubao_asr_api_key`（或 `volc_speech_api_key`）+ `doubao_asr_resource_id` + 可选「模型版本」枚举；链接到官方开通页 |
| 5. Electron 权限 | `main.ts` `session.defaultSession.setPermissionRequestHandler` | S | 对 `media` 请求显式 **allow**（仅限 Soul 自己的 `file://`/`devtools:` 来源）或引导用户到系统设置 |
| 6. 文档 + 冒烟 | `desktop-app/docs/doubao-asr-streaming.md` | S | 密钥申请、resource id 选取、16k 分包、故障码、与 #7 trustTier **无关**（用户口述不触发工具） |

#### 风险

- ⚠️ **协议复杂度高**：gzip、大端、多类 message type，错误实现会导致「连上无字」；必须用官方示例对照 + 单测锁定二进制包。
- ⚠️ **密钥轮换与多账号**：本期只做单组 Key；企业多租户不展开。
- ⚠️ **延迟与功耗**：200ms 分包 vs UI 响应；长时间按住需上限（如 5min）自动 stop。
- ⚠️ **Linux / 沙箱环境**：麦克风权限与 `portal` 行为差异大，完成标准中单独列出「最佳努力」。

#### 完成标准

- 配置 Key + Resource Id 后，在对话页点击 **麦克风** → 说话 → **输入框流式出现中文转写** → 松手或点停止 → 可正常发送消息。
- **主进程外发**请求头含鉴权；渲染进程 **Network 面板不可见** `X-Api-Key`（因不经由 renderer WebSocket）。
- `tsc --noEmit`（desktop-app + core）0 errors；协议模块 **≥ 8** 个单测；`eslint` 新增文件 0 errors（预存问题不扩散）。
- 文档使新同事 **30 分钟内**完成火山控制台开通 + Soul 内配置 + 一次成功识别。

#### 明确不做（本期）

- ❌ 系统级全局语音输入 / 其它应用上屏（启动卡：可选 follow-up）
- ❌ 经典 `volc.streamingasr.*` 非大模型流式 API（除非产品改需求）
- ❌ 修改 `proxy-server.ts` / `widget-server.ts` / `knowledge-retriever.ts`
...
| 2026-05-09 | v1.8 | 追加 **§4.14 P3 WebDAV 跨设备同步**：走 Cherry Studio 全量 zip 快照路线（不走 Remotely-Save 真正双向同步）+ `sync_history` 表（v11→v12）+ `desktop-app/electron/sync/`（webdav-client / credential-store / snapshot-builder / sync-manager）+ 凭据走 safeStorage（与 GitHub PAT 同模式）+ 复用 #11 cron-scheduler 做自动间隔 + 单 zip ≤ 500MB（坚果云上限）+ MVP 不内置 E2EE（推荐 HTTPS）+ MVP 无双向 merge（用户明确选 backup 或 restore）；新依赖 `webdav@^5.9.0` + `archiver` | zhi.qu |
| 2026-05-10 | v1.9 | 追加 **§4.15 P2 流式语音输入（豆包大模型流式 ASR）**：WebSocket `wss://openspeech.bytedance.com/api/v3/sauc/bigmodel*` + 主进程持连接 + IPC 传 16k PCM 分片 + `MessageInput` 集成；官方主文档 `6561/1354869`；与 `proxy-server` / RAG / OCR 解耦 | zhi.qu |
